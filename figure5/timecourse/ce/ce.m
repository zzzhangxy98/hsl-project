...
for seed=1:maxseed
    ...
    opt1.im=1;
    opt1.is=1;
    fid1=fopen(['ce_s_seed_',num2str(seed),'.txt'],'wt');
    jilutmp=[];
    for j=1:parapieces
        opt1.ce=opt1.ce+pararange/(parapieces);
        for i=1:relaxstep
            Fgo=F_func(xn,opt1);
            Frand=T*randn(n,1);
...
save('cesjilu.mat',"jilufinal","jilumid1")
...
for seed=1:maxseed
    ...
    opt1.im=1;
    opt1.is=1;
    fid1=fopen(['ce_d_seed_',num2str(seed),'.txt'],'wt');
%% relax
    for i=1:maxrleaxstep
        Fgo=F_func(xn,opt1);
        Frand=T*randn(n,1);
